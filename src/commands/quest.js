...
const { random, throwDie, multipleThrowDie } = require('../utils')
  ...
  async execute(message) {
    ...
    if(user) {
      ...
      if(collection_01.first().author.id === message.author.id) {
        ...
        await Quest.create({ monsterId: quest.monster.id, nbMonster: quest.nbMonster, challenge: quest.challenge, coins: quest.coins })
      }
    } else {      
